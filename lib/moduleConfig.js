...
export const DEFAULT_MODULE_CONFIG = {
  ...
  crm: {
    ...
    // Custom field definitions (Phase D): [{ key, label, type, options }].
    // type: 'text' | 'number' | 'date' | 'select'. Values live per record
    // in its custom_fields jsonb. Stock has none.
    fields: [],
  },
  ...
  projects: {
    ...
    // Feature toggles — false hides the surface for teams on this variant.
    features: { gantt: true, dependencies: true, checklists: true, budgetSplit: true },
    // Kanban columns a NEW project starts with. null = the app default
    // (lib/boardColumns.js); a list must keep the 'todo'/'done' anchors —
    // resolveBoardColumns re-adds them defensively either way.
    defaultColumns: null,
    fields: [],
  },
  support: {
    ...
    // Label overrides only — status/priority IDS are semantic (SLA logic,
    // automations, "open case" queries) and tones stay with the meaning.
  ...
  invoices: {
    ...
    numberPrefix: 'INV', // e.g. "HVAC" → HVAC-2026-0001
    // Tax defaults pre-filled on NEW invoices (editable per invoice).
    tax: { stateEnabled: false, stateRate: 0, localEnabled: false, localRate: 0 },
    fields: [],
  },
