...
// ── Primary vs secondary vendors ──────────────────────────────────────────
// A PRIMARY vendor supplies a technology's core equipment (Cambium, Ruckus,
// Uniview…) and is what the registry/tabs model. SECONDARY vendors (Vertiv,
// Tripp Lite, Middle Atlantic…) supply hardware-agnostic support gear that
// rides along with ANY primary's build. Registering a vendor on a tech is
// what makes it primary; everything else is secondary by definition.

// Subcategories that define a technology's core equipment — a vendor with
// at least one such product is a primary candidate. Support subcategories
// (UPS, Rack, Cable, Mounting, Storage, licenses…) never make a vendor
// primary on their own.
export const PRIMARY_SUBCATEGORIES = new Set([
  'Access Point',
  'Switch',
  'Aggregate Switch',
  'Gateway',
...
// Distinct vendors that make core equipment for this technology — the Add
// Vendor dropdown's candidate list.
export function primaryVendorCandidates(products, techId) {
  return [
    ...new Set(
      (products ?? [])
        .filter(
          (p) =>
            ...
            p.vendor &&
            PRIMARY_SUBCATEGORIES.has(p.category)
        )
        .map((p) => p.vendor)
    ),
  ].sort();
...
// Whether a catalog product belongs in a given primary vendor's picker:
// the vendor's own products, plus anything NOT owned by another registered
// primary (secondary-vendor and unbranded support gear rides everywhere).
export function productRidesWithVendor(product, vendorName, primaryNames) {
  const v = product?.vendor || '';
  if (!v) return true;
  if (v === vendorName) return true;
  return !(primaryNames ?? []).includes(v);
}
