...
export function resolveBoardColumns(project) {
  const saved = project?.board_columns;
  if (!Array.isArray(saved)) return DEFAULT_BOARD_COLUMNS;
  const cols = saved.filter(
    (c) => c && typeof c.id === 'string' && typeof c.label === 'string' && c.label.trim()
  );
  if (cols.length === 0) return DEFAULT_BOARD_COLUMNS;
  const out = [...cols];
