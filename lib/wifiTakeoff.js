...
//   inUnitSwitchEveryUnit  every unit gets one (a private LAN behind each ONT — FTTU)
...
export function buildWifiTakeoff(model, settingsIn, ctx = {}) {
  ...
  return {
    ...
    inUnitSwitchEveryUnit: !!settings.inUnitSwitchEveryUnit,
    inUnitSwitchSku: String(settings.inUnitSwitchSku ?? '').trim(),
    // Phase 8: the design's architecture — under XGS-PON the engine leaves
    // unit APs off the closet switches (ONT + injector at the AP instead).
