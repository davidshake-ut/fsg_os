...
const INFRASTRUCTURE_PARTS = [
  ...
  infra('PLC-1:32', '1:32 PLC splitter cassette, SC/APC, 1U rack mount', 'Splitter', '', 235.0),
...
// ── Digital Infrastructure: XGS-PON active gear (Phase 8) ───────────────
// The Cambium PON line the Muze FTTU option priced: cost = the quoted
// vendor cost, price = list. Support subscriptions link to their device so
// cost-plus prices them at the device's markup (lib/ponTakeoff.js).
const pon = (sku, desc, category, cost, price, extra = {}) => ({
  sku,
  desc,
  category,
...
const PON_PARTS = [
  pon('TCX16-0A00', 'Cambium TCX16 16-port XGS-PON OLT', 'OLT', 8588.42, 12496.15, { pon_ports: 16, license_sku_5yr: 'CCADV-SUP-TCX16-5' }),
  pon('PSU-OLT-AC-0A', 'Cambium OLT AC power supply module', 'OLT', 354.42, 515.68),
  pon('SFP-XGS-CP-0A', 'Cambium XGS-PON SC/APC OLT optic', 'PON Optic', 324.05, 471.49),
  pon('CCADV-SUP-TCX16-5', 'Cambium Care Advanced — TCX16 OLT, 5-year', 'License', 4332.0, 6303.06),
  pon('SFP-10G-LR', 'Cambium 10G SFP+ SMF LR transceiver (10 km) — OLT uplink', 'Fiber Module', 109.0, 158.6),
  pon('SXX00-0A01', 'Cambium SXX00 XGS-PON indoor ONT', 'ONT', 117.16, 170.47, { license_sku_5yr: 'CCADV-SUP-SXX00-5' }),
  pon('CCADV-SUP-SXX00-5', 'Cambium Care Advanced — SXX00 ONT, 5-year', 'License', 36.0, 52.38),
  pon('C000000L141A', 'Cambium 60 W PoE injector (AP power at the ONT)', 'PoE Injector', 28.24, 41.09),
  pon('N000900L031A', 'AC power cord for PoE injector', 'PoE Injector', 3.95, 5.75),
  pon('SXS00-0A00', 'Cambium SXS00 XGS-PON multi-port ONU', 'ONT', 150.0, 218.25),
];

export const BASE_PRODUCTS = [...RAW_BASE_PRODUCTS, ...INFRASTRUCTURE_PARTS, ...PON_PARTS, ...CABLING_RUNS, ...INFRASTRUCTURE_KITS].map((p) => ({
  ...p,
...
export const CATEGORY_ORDER = [
  ...
  'Rack Accessory', 'Fiber', 'Enclosure',
  // XGS-PON (Phase 8)
  'OLT', 'ONT', 'PON Optic', 'Splitter', 'PoE Injector',
  'Miscellaneous', 'Cabling', 'Software',
  // Camera systems
