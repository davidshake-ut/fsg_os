...
// opts.staleSentDays: variant-tunable window before a sent proposal counts
// as "no movement" (CRM module config, default 7).
export function deriveNextSteps({ account, contacts = [], quotes = [], projects = [], tickets = [], invoices = [] }, now = Date.now(), opts = {}) {
  const STALE_SENT_DAYS = Math.max(1, Number(opts.staleSentDays) || 7);
  const steps = [];
