...
export function calculateBOM(
  ...
  if (useTakeoff) {
    for (const room of takeoffRooms) {
      const aps = Math.max(0, Math.round(Number(room.aps) || 0));
    ...
    // Townhomes: every unit hosts its own small PoE switch (no shared closet).
    if (takeoff.switchPerTownhome) {
  ...
  const inUnitSwitches =
    useTakeoff && takeoff.inUnitSwitchForMultiAp ? Math.max(0, Math.round(Number(takeoff.multiApUnits) || 0)) : 0;
  if (inUnitSwitches > 0 && takeoff.inUnitSwitchSku) {
    addItem(takeoff.inUnitSwitchSku, inUnitSwitches, 'In-unit switch (units with 2+ APs)');
  }
