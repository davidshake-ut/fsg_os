...
//   kits      see KIT_DEFAULTS (Phase 3);  cabling  see lib/cablingRuns.js (Phase 4)

import { CABLING_DEFAULTS, CABLING_RUN_KEYS } from './cablingRuns';
import { PON_DEFAULTS, normalizePon, normalizeArchitecture } from './ponModel';
...
export const PROPERTY_MODEL_DEFAULTS = {
  ...
  notes: '',
  kits: KIT_DEFAULTS,
  cabling: CABLING_DEFAULTS,
  // Phase 8: 'active_ethernet' | 'xgs_pon' and the PON sizing rules.
  architecture: 'active_ethernet',
  pon: PON_DEFAULTS,
};
...
export function normalizePropertyModel(value) {
  ...
  // Cabling (Phase 4): per run type, an optional SKU, an entered quantity
  // (null = derived), and an enabled flag. Unknown run keys drop out.
  const cIn = v.cabling && typeof v.cabling === 'object' ? v.cabling : {};
  const runs = {};
  for (const key of CABLING_RUN_KEYS) {
    const r = cIn.runs && typeof cIn.runs === 'object' ? cIn.runs[key] : null;
    if (!r || typeof r !== 'object') continue;
    const entry = {};
    if (clean(r.sku)) entry.sku = clean(r.sku);
    if (r.qty !== null && r.qty !== undefined && r.qty !== '' && Number.isFinite(Number(r.qty))) entry.qty = n0(r.qty);
    if (r.enabled === false) entry.enabled = false;
    if (Object.keys(entry).length) runs[key] = entry;
  }
  const cabling = { enabled: cIn.enabled !== false, runs };
  ...
  return {
    ...
    kits,
    cabling,
    architecture: normalizeArchitecture(v.architecture),
    pon: normalizePon(v.pon),
  };
}
