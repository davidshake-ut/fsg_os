...
export function calculateCameraBOM(
  ...
  // catalogSnapshot (via shipping.catalogSnapshot) so a later catalog/discount
  // change never silently reprices it — only drafts/revisions read live data.
  function getProduct(sku) {
    ...
    const base = shipping.catalogSnapshot?.[sku]
  ...
  function addItem(sku, qty, note = '') {
    // Per-line overrides (BOM table edit mode) — same shape as calculateBOM:
    // cost/price via getProduct plus qty, display sku, description, removal.
    ...
    items.push({
      sku: ov?.sku ?? p.sku,
      baseSku: sku, // identity, not the (possibly aliased) display sku
      description: ov?.description ?? p.desc,
      qty: effQty,
      unitCost: p.cost,
      unitPrice: p.price,
      totalCost: p.cost * effQty,
      totalPrice: p.price * effQty,
      total: p.price * effQty,
