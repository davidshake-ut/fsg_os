...
export function buildScopeOfWork({ inputs, cameraInputs, wifiBom, cameraBom, term }) {
  ...
  if (wifiBom.totalAPs > 0) {
    ...
    const deploy =
      inputs.deploymentType === 'inroom' || inputs.deploymentType === 'wall'
        ? 'wall-mounted'
        : 'ceiling-mounted';
    // Takeoff mode carries the unit and telecom-room counts on the BOM.
    const idfs = Math.max(1, Number(wifiBom.idfCount ?? inputs.numberOfIDFs) || 1);
    const units = wifiBom.unitCount ?? inputs.numberOfRooms;

    blocks.push({
      title: 'Managed Wi-Fi Network',
      text:
        `A fully managed ${gen} wireless network engineered to deliver reliable, high-density coverage ` +
        `for ${plural(units, unit)}. The design deploys ${plural(wifiBom.totalAPs, 'Wi-Fi access point')} ` +
