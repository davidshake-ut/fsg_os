...
// Tax lines for display/export: state and/or local when the split fields
// (0059) are enabled, otherwise the legacy combined rate. The last line is
// nudged so the lines always sum exactly to the stored tax_amount.
export function invoiceTaxLines(inv) {
  const subtotal = Number(inv.subtotal ?? 0);
  const taxAmount = round2(Number(inv.tax_amount ?? 0));
  const lines = [];
  if (inv.state_tax_enabled) {
    const rate = Number(inv.state_tax_rate) || 0;
    lines.push({ label: `State Tax (${rate}%)`, amount: round2(subtotal * rate / 100) });
  }
  if (inv.local_tax_enabled) {
    const rate = Number(inv.local_tax_rate) || 0;
    lines.push({ label: `Local Tax (${rate}%)`, amount: round2(subtotal * rate / 100) });
  }
  if (!lines.length && Number(inv.tax_rate) > 0) {
    lines.push({ label: `Tax (${inv.tax_rate}%)`, amount: taxAmount });
  }
  if (lines.length) {
    const drift = round2(taxAmount - lines.reduce((s, l) => s + l.amount, 0));
...
export function exportInvoiceCSV(inv) {
  ...
  const rows = [
    ...
    ...invoiceTaxLines(inv).map((l) => [l.label, '', '', l.amount.toFixed(2)]),
...
export function exportInvoicePDF(inv, branding = {}) {
  ...
  const totalRows = [
    ...
    ...invoiceTaxLines(inv).map((l) => [l.label, money(l.amount)]),
