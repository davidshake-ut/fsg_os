...
import { isAlternate, optionTags, optionGroups, optionTitle, comparisonRows, signed } from './vendorComparison';
...
export function exportPDF(inputs, sections, opts = {}) {
  ...
  if (bannerLogo?.dataUrl) {
    try {
      ...
      doc.addImage(bannerLogo.dataUrl, fmt, pageW - 10 - drawW, (28 - drawH) / 2, drawW, drawH);
    } catch {
  ...
  doc.setFontSize(8);
  doc.text(`Generated: ${fmtDate(new Date())}`, pageW - 10, 33, { align: 'right' });

  const ensureSpace = (y, needed) => (y + needed > pageH - 12 ? (doc.addPage(), 18) : y);

  const sectionBar = (label, y, fill = primary, text = headText) => {
  ...
  let grandCost = 0;
  let grandPrice = 0;
  // Option-B alternates (a multi-vendor tech's non-primary vendors) render
  // as their own accent-barred section but never reach the project total.
  ...
  for (const section of sections) {
    ...
    y = ensureSpace(y, 40);
    y = alternate
      ? sectionBar(optionTitle(section, tags), y, accent, accentText)
      : sectionBar(optionTitle(section, tags), y);
    ...
    autoTable(doc, {
      ...
      foot: [[
        alternate ? `${section.title} Subtotal (Alternate — not in project total)` : `${section.title} Subtotal`,
        money(bom.grandTotalPrice),
      ]],
      headStyles: { fillColor: primary, textColor: headText, fontSize: 8 },
